<template>
  <div class="app-container">
    ...
    <el-row :gutter="0" type="flex">
      <el-col class="toolbar">
        <el-form :inline="true">
          ...
          <el-form-item>
            <el-button size="small" el-button icon="el-icon-refresh" circle @click="getPhraseList" />
          </el-form-item>
        </el-form>
      </el-col>
    </el-row>
    <!-- 表格列表 -->
    <el-table v-loading="listLoading" :data="phrase" border stripe row-key="id" highlight-current-row>
      <el-table-column type="selection" :reserve-selection="true" align="center" />
      <el-table-column v-if="false" align="center" prop="id" label="台词ID" sortable />
      ...
      <el-table-column align="center" prop="content" label="台词内容" :show-overflow-tooltip="true">
        <template slot-scope="props">
          <span class="col-cont" v-html="searchKeyColorChange(props.row.content)" />
        </template>
      </el-table-column>
    ...
    <!-- 新增 -->
    <el-dialog title="新增台词信息" :visible.sync="addFormVisible" :close-on-click-modal="false">
      <el-form ref="addForm" :model="addForm" label-width="120px">
        ...
        <el-form-item label="台词内容" prop="content" required>
          <el-input
    ...
    <!-- 编辑 -->
    <el-dialog title="编辑台词信息" :visible.sync="editFormVisible" :close-on-click-modal="false">
...
<script>
import { getPhrases, createPhrase, updatePhrase, deletePhrase } from '../../api/movie/phrase.js'
  ...
  methods: {
    ...
    addSubmit: function() {
      this.$refs.addForm.validate((valid) => {
        if (valid) {
          this.$confirm('确认添加此条台词信息吗？', '提示', {}).then(() => {
            const data = Object.assign({}, this.addForm)
            createPhrase(data).then((response) => {
    ...
    editSubmit: function() {
      this.$refs.editForm.validate((valid) => {
        if (valid) {
          ...
          this.$confirm('确认修改台词信息吗？', '提示', {}).then(() => {
            this.editLoading = true
            const data = Object.assign({}, this.editForm)
            updatePhrase(data).then((response) => {
    ...
    deleteSingleMedia: function(rowId) {
      this.$confirm('确认删除该条台词信息吗？', '提示', { type: 'warning' }).then(() => {
        this.listLoading = true
        deletePhrase(rowId).then((response) => {
          this.listLoading = false
          this.$message({
            message: '台词信息删除成功!',
            type: 'success'
